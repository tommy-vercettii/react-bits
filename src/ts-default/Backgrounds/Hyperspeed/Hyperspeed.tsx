...
interface DistortionUniforms {
  uFreq: { value: THREE.Vector2 | THREE.Vector3 | THREE.Vector4 | any }
  uAmp: { value: THREE.Vector2 | THREE.Vector3 | THREE.Vector4 | any }
  uPowY?: { value: THREE.Vector2 | any }
}

interface DistortionJS {
  (progress: number, time: number): THREE.Vector3
}
...
interface EffectOptions {
  onSpeedUp?: (e?: MouseEvent) => void
  onSlowDown?: (e?: MouseEvent) => void
  distortion: any
  length: number
  roadWidth: number
  islandWidth: number
...
interface ThreeColor extends THREE.Color {
  r: number
  g: number
  b: number
}
...
class App {
  ...
  constructor (container: HTMLDivElement | HTMLElement, options: EffectOptions) {
  ...
  init (): void {
    ...
    this.container.addEventListener('mousedown', ((e: Event) =>
      this.onMouseDown(e as MouseEvent)) as EventListener)
    this.container.addEventListener('mouseup', ((e: Event) =>
      this.onMouseUp(e as MouseEvent)) as EventListener)
  ...
  setSize (width: number, height: number, updateStyles?: boolean): void {
    this.composer.setSize(width, height, updateStyles ?? false)
  }
...
class CarLights implements CarLightsProps {
  webgl: App
  options: EffectOptions
  colors: number[] | number
  speed: [number, number]
  ...
  init (): void {
    ...
    Object.assign(instanced, geometry)
    ...
    let colorComponents: ColorComponents[] = colorArray.map(c => {
    ...
    for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
      ...
      let color = pickRandom(colorComponents)
  ...
  update (time: number): void {
    if (this.mesh) {
      ;(this.mesh.material as ShaderMaterialWithUniforms).uniforms.uTime.value =
        time
    }
  }
}
...
class LightsSticks implements LightsStickProps {
  webgl: App
  options: EffectOptions
  mesh?: THREE.Mesh
...
class Road implements RoadProps {
  webgl: App
  options: EffectOptions
  uTime: { value: number }
  leftRoadWay?: THREE.Mesh
...
const Hyperspeed: React.FC<HyperspeedProps> = ({
  ...
  useEffect(() => {
    ...
    let nsin: any = (val: number) => Math.sin(val) * 0.5 + 0.5
    ...
    const distortions: Record<string, Distortion> = {
      ...
      turbulentDistortion: {
        ...
        getJS: (progress, time) => {
          ...
          const getX = (p: number) =>
            Math.cos(Math.PI * p * uFreq.r + time) * uAmp.r +
            Math.pow(
              Math.cos(Math.PI * p * uFreq.g + time * (uFreq.g / uFreq.r)),
              2
            ) *
              uAmp.g

          const getY = (p: number) =>
            -nsin(Math.PI * p * uFreq.b + time) * uAmp.b -
            Math.pow(
              nsin(Math.PI * p * uFreq.a + time / (uFreq.z / uFreq.w)),
              5
            ) *
              uAmp.w
    ...
    const distortionKey = effectOptions.distortion as DistortionType
    const selectedDistortion = distortions[distortionKey as any]
    if (selectedDistortion) {
      effectOptions.distortion = selectedDistortion
    }
